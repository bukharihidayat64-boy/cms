...
<div class="w-full space-y-6 animate-fade-in">
        ...
        <div class="relative z-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-white/80 text-sm mt-1">Kelola pengguna yang terdaftar di sistem</p>
                </div>
            </div>
            {{-- Tidak ada tombol tambah karena user register via frontend --}}
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
        <form method="GET" class="flex flex-col md:flex-row gap-4">
            ...
            <a href="{{ route('admin.users.index') }}" class="px-6 py-3 rounded-xl font-semibold text-gray-600 hover:bg-gray-100 border-2 border-gray-200 transition-all">Reset</a>
        </form>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
            ...
            <table class="w-full">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider w-64">Email</th>
                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider w-32">Role</th>
                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider w-32">Status</th>
                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider w-40">Terdaftar</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="group hover:bg-secondary/5 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-full bg-gradient-to-br from-secondary to-secondary-container flex items-center justify-center text-white font-bold shadow-sm flex-shrink-0">
                                    {{ strtoupper(substr($user->name, 0, 1)) }}
                                </div>
                                <div class="min-w-0">
                                    <p class="font-semibold text-gray-900 truncate group-hover:text-secondary transition-colors">{{ $user->name }}</p>
                                    @if($user->phone)
                        ...
                        <td class="px-6 py-4">
                            <span class="px-3 py-1 rounded-full text-xs font-bold {{ $user->role == 'admin' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700' }}">
                        ...
                        <td class="px-6 py-4">
                            ...
                            @else
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-gray-100 text-gray-600">Inactive</span>
                            @endif
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-600">{{ $user->created_at->format('d M Y') }}</td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                <a href="{{ route('admin.users.edit', $user) }}" class="inline-flex items-center gap-1.5 px-3 py-2 bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-lg hover:from-blue-600 hover:to-blue-700 transition-all shadow-sm hover:scale-105 active:scale-95 text-sm font-medium">
                    ...
                    <tr>
                        <td colspan="7" class="px-6 py-12 text-center">
                            <span class="material-symbols-outlined text-5xl text-gray-300 mb-3 block">people</span>
        ...
        <div class="px-6 py-4 border-t border-gray-200 bg-gray-50 flex justify-between items-center">
            <p class="text-sm text-gray-600">Menampilkan <span class="font-bold text-secondary">{{ $users->firstItem() }}</span> - <span class="font-bold text-secondary">{{ $users->lastItem() }}</span> dari <span class="font-bold text-secondary">{{ $users->total() }}</span></p>
            {{ $users->links() }}
        </div>
        @endif
    </div>
