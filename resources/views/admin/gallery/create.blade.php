...
<div class="w-full space-y-6 animate-fade-in">
    ...
    <div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-secondary via-secondary-container to-primary-container p-8 text-white shadow-2xl">
        ...
        <div class="relative z-10">
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-3xl font-bold tracking-tight">Upload Foto Baru</h1>
                    <p class="text-white/80 text-sm mt-1">Tambahkan dokumentasi ekspedisi ke galeri</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
        <form action="{{ route('admin.gallery.store') }}" method="POST" enctype="multipart/form-data" class="p-8">
            @csrf
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                {{-- Upload Area --}}
                <div class="lg:col-span-1">
                    <label class="block text-sm font-bold text-gray-700 uppercase tracking-wider mb-3">File Foto <span class="text-red-500">*</span></label>
                    <div class="relative group">
                        <input type="file" name="image" id="imageInput" accept="image/*" class="hidden" onchange="previewFile(this)" required>
                        <label for="imageInput" id="dropZone" class="flex flex-col items-center justify-center w-full h-64 border-2 border-dashed border-gray-300 rounded-xl cursor-pointer bg-gray-50 hover:border-secondary hover:bg-secondary/5 transition-all">
                            <img id="preview" class="hidden w-full h-full object-cover rounded-lg mb-2">
                            <div id="placeholder" class="flex flex-col items-center text-center p-4">
                                <span class="material-symbols-outlined text-4xl text-gray-400 mb-2">cloud_upload</span>
                                <span class="text-sm font-medium text-gray-600">Klik atau drag foto ke sini</span>
                ...
                <div class="lg:col-span-2 space-y-5">
                    <div>
                        ...
                        <input type="text" name="title" value="{{ old('title') }}" required minlength="3" placeholder="Contoh: Sunrise di Plawangan" 
    ...
    <select name="category" required
        ...
        <option value="summit" {{ old('category') == 'summit' ? 'selected' : '' }}>Summit</option>
        <option value="basecamp" {{ old('category') == 'basecamp' ? 'selected' : '' }}>Basecamp</option>
        <option value="flora_fauna" {{ old('category') == 'flora_fauna' ? 'selected' : '' }}>Flora & Fauna</option>
        <option value="tim_ekspedisi" {{ old('category') == 'tim_ekspedisi' ? 'selected' : '' }}>Tim Ekspedisi</option>
    ...
    @error('category') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            ...
                            <input type="text" name="photographer" value="{{ old('photographer') }}" placeholder="Nama fotografer" 
                        ...
                        <div>
                            ...
                            <input type="text" name="location" value="{{ old('location') }}" placeholder="Lokasi foto" 
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="4" placeholder="Tambahkan deskripsi singkat..." maxlength="500"
                                  class="w-full px-4 py-3 bg-gray-50 border-2 border-gray-200 rounded-xl focus:outline-none focus:border-secondary focus:bg-white transition-all resize-none @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                        @error('description') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label class="block text-sm font-bold text-gray-700 uppercase tracking-wider mb-2">Tags</label>
                        <input type="text" name="tags" value="{{ old('tags') }}" placeholder="Contoh: sunrise, rinjani, plawangan (pisahkan dengan koma)" 
                        ...
                        <div>
                            ...
                            <div class="flex gap-3">
                                <label class="flex-1 cursor-pointer">
                                    <input type="radio" name="is_featured" value="1" {{ old('is_featured') ? 'checked' : '' }} class="peer sr-only">
                                ...
                                <label class="flex-1 cursor-pointer">
                                    <input type="radio" name="is_featured" value="0" {{ !old('is_featured') ? 'checked' : '' }} class="peer sr-only">
            ...
            <div class="flex items-center justify-end gap-4 pt-6 border-t border-gray-200 mt-6">
                ...
                <button type="submit" class="group inline-flex items-center gap-2 bg-gradient-to-r from-blue-500 to-blue-600 text-white px-6 py-3 rounded-xl font-semibold hover:shadow-lg hover:scale-[1.02] active:scale-95 transition-all shadow-blue-500/30">
                    <span class="material-symbols-outlined group-hover:rotate-12 transition-transform">save</span> Simpan Foto
                </button>
            </div>
        </form>
    </div>
