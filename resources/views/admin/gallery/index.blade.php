...
<div class="w-full space-y-6 animate-fade-in">
    ...
    <div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-secondary via-secondary-container to-primary-container p-8 text-white shadow-2xl">
        ...
        <div class="relative z-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-3xl font-bold tracking-tight">Galeri</h1>
                    <p class="text-white/80 text-sm mt-1">Kelola koleksi foto ekspedisi Gunung Rinjani</p>
                </div>
            </div>
            <a href="{{ route('admin.gallery.create') }}" class="group inline-flex items-center gap-2 bg-white text-secondary px-6 py-3 rounded-xl font-semibold hover:bg-secondary-fixed transition-all shadow-lg hover:shadow-2xl hover:scale-105 active:scale-95">
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
        <form method="GET" class="flex flex-col md:flex-row gap-4">
            <div class="flex-1 relative">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari judul atau fotografer..." 
            ...
            <a href="{{ route('admin.gallery.index') }}" class="px-6 py-3 rounded-xl font-semibold text-gray-600 hover:bg-gray-100 border-2 border-gray-200 transition-all">Reset</a>
        </form>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="group hover:bg-secondary/5 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                <div class="w-16 h-16 rounded-lg overflow-hidden bg-gray-100 flex-shrink-0 ring-2 ring-gray-100 group-hover:ring-secondary/30 transition-all">
                                    @if($gallery->image)
                                        {{-- Pastikan path sesuai (biasanya storage/ untuk disk public) --}}
                                ...
                                <div class="min-w-0">
                                    <p class="font-semibold text-gray-900 truncate group-hover:text-secondary transition-colors">{{ $gallery->title ?? 'Tanpa Judul' }}</p>
                                    <p class="text-xs text-gray-500 truncate">{{ Str::limit($gallery->description, 40) }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            @php
                                // Format kategori agar 'flora_fauna' menjadi 'Flora Fauna'
                                $catDisplay = $gallery->category ? ucwords(str_replace('_', ' ', $gallery->category)) : 'Umum';
                            @endphp
                            <span class="px-3 py-1 rounded-full text-xs font-bold bg-gradient-to-r from-purple-400 to-pink-400 text-white shadow-sm">
                                {{ $catDisplay }}
                            </span>
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                <a href="{{ route('admin.gallery.edit', $gallery->id) }}" class="inline-flex items-center gap-1.5 px-3 py-2 bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-lg hover:from-blue-600 hover:to-blue-700 transition-all shadow-sm hover:scale-105 active:scale-95 text-sm font-medium">
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center">
                            <span class="material-symbols-outlined text-5xl text-gray-300 mb-3 block">photo_library</span>
        ...
        <div class="px-6 py-4 border-t border-gray-200 bg-gray-50 flex justify-between items-center">
            <p class="text-sm text-gray-600">Menampilkan <span class="font-bold text-secondary">{{ $galleries->firstItem() }}</span> - <span class="font-bold text-secondary">{{ $galleries->lastItem() }}</span> dari <span class="font-bold text-secondary">{{ $galleries->total() }}</span></p>
            {{ $galleries->links() }}
        </div>
        @endif
    </div>
