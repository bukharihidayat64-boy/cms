...
<head>
    ...
    <title>Register | RinjaniTrail.id</title>
    ...
    <style>
        ...
        .logo-icon {
            ...
            box-shadow: 0 10px 30px rgba(190, 234, 248, 0.3), inset 0 1px 0 rgba(255, 255, 255, 0.3);
...
<div class="mountain-bg w-full min-h-screen flex items-center justify-center relative">
    ...
    <!-- Register Card -->
    <div class="register-card w-full max-w-md rounded-3xl p-8 md:p-10 relative z-10 my-10">
        ...
        <div class="text-center mb-8 fade-in-1">
            ...
            <h1 class="font-headline text-3xl font-bold text-white mb-2">Join the Adventure</h1>
            <p class="text-secondary-fixed text-sm font-medium">Bergabung dengan komunitas pendaki Rinjani</p>
        </div>
        
        <!-- Success Message -->
        @if(session('success'))
            <div class="mb-6 p-4 bg-green-500/10 border border-green-500/30 rounded-xl flex items-start gap-3 fade-in-2 backdrop-blur-sm">
                <span class="material-symbols-outlined text-green-400 text-[20px] flex-shrink-0">check_circle</span>
        ...
        <!-- Error Messages -->
        @if($errors->any())
            <div class="mb-6 p-4 bg-red-500/10 border border-red-500/30 rounded-xl flex items-start gap-3 fade-in-2 backdrop-blur-sm">
                <span class="material-symbols-outlined text-red-400 text-[20px] flex-shrink-0">error</span>
                <div class="flex-1">
                    <p class="text-red-300 font-semibold text-sm">Registrasi Gagal</p>
        ...
        <!-- Register Form -->
        <form action="{{ route('user.register.process') }}" method="POST" class="space-y-4">
            @csrf
            
            <!-- Full Name -->
            <div class="fade-in-2">
                <label class="block text-white/70 text-xs font-medium mb-2 uppercase tracking-wider">Full Name</label>
                <div class="input-group">
                    <span class="material-symbols-outlined input-icon absolute left-4 top-1/2 -translate-y-1/2 text-white/50 text-[20px]">person</span>
                    <input type="text" 
                           name="name" 
                           value="{{ old('name') }}"
                           required 
            ...
            <div class="fade-in-3">
                ...
                <div class="input-group">
                    ...
                    <button type="button" 
                            id="toggle-password" 
                            class="absolute right-4 top-1/2 -translate-y-1/2 text-white/50 hover:text-secondary-fixed transition-all duration-300 hover:scale-110 focus:outline-none z-10">
            ...
            <!-- Confirm Password -->
            <div class="fade-in-4">
                <label class="block text-white/70 text-xs font-medium mb-2 uppercase tracking-wider">Confirm Password</label>
                <div class="input-group">
                    <span class="material-symbols-outlined input-icon absolute left-4 top-1/2 -translate-y-1/2 text-white/50 text-[20px]">lock_clock</span>
                    <input type="password" 
                    ...
                    <button type="button" 
                            ...
                            class="absolute right-4 top-1/2 -translate-y-1/2 text-white/50 hover:text-secondary-fixed transition-all duration-300 hover:scale-110 focus:outline-none z-10">
                        <span class="material-symbols-outlined text-[20px]" id="eye-icon-confirm">visibility</span>
                    </button>
                </div>
            </div>
            
            <!-- Terms & Conditions -->
            <div class="fade-in-4 flex items-start gap-2">
                <input type="checkbox" name="terms" id="terms" required class="custom-checkbox mt-1">
                <label for="terms" class="text-white/70 text-xs cursor-pointer">
                    Saya setuju dengan <a href="#" class="link-hover text-secondary-fixed font-medium">Syarat & Ketentuan</a> serta <a href="#" class="link-hover text-secondary-fixed font-medium">Kebijakan Privasi</a> RinjaniTrail.id
                </label>
            </div>
        ...
        <div class="relative my-6 fade-in-5">
            ...
            <div class="relative flex justify-center text-xs">
                <span class="bg-transparent px-4 text-white/40">Sudah punya akun?</span>
            </div>
        </div>
        
        <!-- Back to Login -->
        <div class="text-center fade-in-5">
            <a href="{{ route('user.login') }}" class="link-hover inline-flex items-center gap-2 text-white/60 hover:text-secondary-fixed text-sm font-medium transition-colors">
...
<script>
    ...
    if (toggleBtn && passwordInput) {
        toggleBtn.addEventListener('click', () => {
            ...
            eyeIcon.style.transform = 'scale(0.5) rotate(180deg)';
            setTimeout(() => {
    ...
    if (toggleConfirmBtn && confirmPasswordInput) {
        toggleConfirmBtn.addEventListener('click', () => {
            ...
            eyeIconConfirm.style.transform = 'scale(0.5) rotate(180deg)';
            setTimeout(() => {
                eyeIconConfirm.textContent = isPassword ? 'visibility_off' : 'visibility';
                eyeIconConfirm.style.transform = 'scale(1) rotate(0deg)';
            }, 150);
        });
    }
</script>
